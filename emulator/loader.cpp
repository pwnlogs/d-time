...
// Load blocks and meta from files
/*
...
{   
	...
	// Global\\ removed to make D-TIME work without Admin-previlages
    sprintf_s(fullregnam,MAXSEGNAMELEN + 16, "%hs", regnam);
...
#define SEGLEN (shctrl->segm[i].endEA - shctrl->segm[i].startEA)
    ...
    for(uint i=0; fread(&shctrl->segm[i].segmid, 1, 2, stfp) == 2; i++ )                        
    {
        ...
        if( fread(segm, 1, SEGLEN, segfp) != SEGLEN )       // copy segment to shared region
